...
Go to <http://cran.r-project.org/bin/windows/Rtools/> and download the latest version (e.g., Rtools40.exe).
...
To enable this kernel in the notebooks,
open R **from the Windows Terminal** (not R Studio) and run the setup via the following two commands:
...
> **Note:** You cannot use RStudio for this step because it will not be able to find the jupyter installation
>  R from terminal will since the correct PATH for jupyter is set when the terminal is launched.
...
![](/resources_pages/imgs/extract_make.png)

![](/resources_pages/imgs/extract_make_path.png)
