...
3. Add at least one new R package to the Dockerfile using (ensure that you pin the version of the package you add).
    ...
    - Another way is to `RUN` commands directly in the `Dockerfile` to install packages
        ...
        - Hint: You can run R code directly from the command line. For example, `R -e "print('something to say...')"`. Make sure to change the double quotes `""` to single quotes `''` in the R code.  
...
6. Make sure you make at least one change to the `Dockerfile` once the GitHub Actions have been setup
...
4. The Docker image version tag is in sync with the GitHub repository release tag.
