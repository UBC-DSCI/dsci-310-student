...
will make moving on to the next one easier. The best way to do so is via the project
board that you are already required to use. 

### 1. Abstract code from your literate code document (`*.ipynb`, `*.Rmd`, or `.qmd`) to scripts (e.g., `.R` or `.py`).
...
### 2. Convert your `*.ipynb` or `*.Rmd` files into a Quarto document (`*.qmd`). Edit your Quarto document so that it's sole job is to narrate your analysis, display your analysis artifacts (i.e., figures and tables), and nicely format the report
...
- have a `clean` target so that you can easily "undo" your analysis by running `make clean` at the command line (e.g., deletes all generated data and files).

Tip:

- The `all` and `clean` targets can be `.PHONY` targets
...
### 4. Continue to manage issues professionally
...
This is a group submission on Gradescope. At least one group member will initiate the submission, add their team members, and then submit the following information.

** Don't forget to add your fellow teammates! **

You will submit two URLs to Gradescope in the provided text box for milestone 2:
