...
> Uninstall the Miniconda you just installed (which usually lives in the `/home/<USER>` directory),
...
however for Jupyter we need to add the TinyTex executables to our `PATH`
and install several more packages
...
(which you can open by `code ~/.bashrc`) and save the file:
...
Next, [follow the Linux post installation steps here](https://docs.docker.com/engine/install/linux-postinstall/)
so that you can run Docker without typing `sudo`
...
OK        git 2.35.1
OK        make 4.3
OK        latex 3.141592653-2.6-1.40.24 (TeX Live 2021)
