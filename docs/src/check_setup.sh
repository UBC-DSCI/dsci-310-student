...
# which are needed for the start of the DSCI 310 course are correctly installed.
...
echo -e "${ORANGE}## Operating system${NC}" >> check-setup-310.log
if [[ "$(uname)" == 'Linux' ]]; then
    ...
    grep "Architecture" <<< $sys_info | sed 's/^[[:blank:]]*//;s/:/:    /' >> check-setup-310.log
    grep "Kernel" <<< $sys_info | sed 's/^[[:blank:]]*//;s/:/:          /' >> check-setup-310.log
    file_browser="xdg-open"
    if ! $(grep -iq "22.04" <<< $os_version); then
        echo '' >> check-setup-310.log
        echo "MISSING You are recommended to use Ubuntu 22.04." >> check-setup-310.log
    fi
    ...
    echo "Operating system verison could not be detected." >> check-setup-310.log
fi
echo '' >> check-setup-310.log
...
echo -e "${ORANGE}## System programs${NC}" >> check-setup-310.log
...
# Rstudio are not on PATH in windows
    ...
    if ! [ "$(command -v tlmgr.bat)" ]; then
        echo "MISSING   tlmgr 5.*" >> check-setup-310.log
    else
...
for sys_prog in ${sys_progs[@]}; do
    ...
    if ! [ -x "$(command -v $sys_prog_no_version)" ]; then
        ...
        echo "MISSING   $sys_prog" >> check-setup-310.log
    else
        ...
        if ! $(grep -iq "$regex_version" <<< "$($sys_prog_no_version --version &> >(head -1))"); then
            ...
            echo "OK        "$sys_prog_no_version $installed_version >> check-setup-310.log
        fi
    fi
done
...
# Currently marks both uninstalled and wrong verion number as MISSING
echo "" >> check-setup-310.log
echo -e "${ORANGE}## Python packages${NC}" >> check-setup-310.log
if ! [ -x "$(command -v conda)" ]; then  # Check that conda exists as an executable program
    echo "Please install 'conda' to check Python package versions." >> check-setup-310.log
    echo "If 'conda' is installed already, make sure to run 'conda init'" >> check-setup-310.log
    echo "if this was not chosen during the installation." >> check-setup-310.log
    ...
    for py_pkg in ${py_pkgs[@]}; do
        ...
        if ! $(grep -iq "$py_pkg" <<< $installed_py_pkgs); then
            echo "MISSING   ${py_pkg}.*" >> check-setup-310.log
        else
            ...
            echo "OK        $(grep -io "${py_pkg}\S*" <<< $installed_py_pkgs)" >> check-setup-310.log
        fi
    done
fi
...
if ! [ -x "$(command -v jupyter)" ]; then  # Check that jupyter exists as an executable program
    echo "Please install 'jupyterlab' before testing PDF generation." >> check-setup-310.log
else
    ...
    if ! jupyter nbconvert mds-nbconvert-test.ipynb --to pdf --log-level 'ERROR' &> jupyter-pdf-error.log; then
        echo 'MISSING   jupyterlab PDF-generation failed. Check that latex and jupyterlab are marked OK above, then read the detailed error message in the log file.' >> check-setup-310.log
    else
...
# Currently marks both uninstalled and wrong verion number as MISSING
echo "" >> check-setup-310.log
echo -e "${ORANGE}## R packages${NC}" >> check-setup-310.log
if ! [ -x "$(command -v R)" ]; then  # Check that R exists as an executable program
    ...
    for r_pkg in ${r_pkgs[@]}; do
        if ! $(grep -iq "$r_pkg" <<< $installed_r_pkgs); then
            echo "MISSING   $r_pkg.*" >> check-setup-310.log
        else
            # Match the package name up until the first whitespace to get regexed versions
            # without getting all following packages contained in the string of all pacakges
            echo "OK        $(grep -io "${r_pkg}\S*" <<< $installed_r_pkgs)" >> check-setup-310.log
        fi
    done
fi
...
if ! [ -x "$(command -v R)" ]; then  # Check that R exists as an executable program
    echo "Please install 'R' before testing PDF and HTML generation." >> check-setup-310.log
else
    ...
    if ! Rscript -e "rmarkdown::find_pandoc(dir = c('/usr/lib/rstudio/resources/app/bin/quarto/bin/tools', 'C:/Program Files/RStudio/resources/app/bin/quarto/bin/tools', '/Applications/RStudio.app/Contents/MacOS/quarto/bin/tools', '/Applications/RStudio.app/Contents/Resources/app/quarto/bin/tools'), cache=F); rmarkdown::render('mds-knit-pdf-test.Rmd', output_format = 'pdf_document')" &> /dev/null; then
        echo "MISSING   rmarkdown PDF-generation failed. Check that latex and rmarkdown are marked OK above." >> check-setup-310.log
    else
        echo 'OK        rmarkdown PDF-generation was successful.' >> check-setup-310.log
    fi
    if ! Rscript -e "rmarkdown::find_pandoc(dir = c('/usr/lib/rstudio/resources/app/bin/quarto/bin/tools', 'C:/Program Files/RStudio/resources/app/bin/quarto/bin/tools', '/Applications/RStudio.app/Contents/MacOS/quarto/bin/tools', '/Applications/RStudio.app/Contents/Resources/app/quarto/bin/tools'), cache=F); rmarkdown::render('mds-knit-pdf-test.Rmd', output_format = 'html_document')" &> /dev/null; then
        echo "MISSING   rmarkdown HTML-generation failed. Check that rmarkdown is marked OK above." >> check-setup-310.log
    else
        echo 'OK        rmarkdown HTML-generation was successful.' >> check-setup-310.log
    fi
    # -f makes sure `rm` succeeds even when the file does not exists
...
tail -n +2 check-setup-310.log  # `tail` to skip rows already echoed to stdout
...
if [ -s jupyter-pdf-error.log ]; then
    echo '' >> check-setup-310.log
    echo '======== You had the following errors during Jupyter PDF generation ========' >> check-setup-310.log
    cat jupyter-pdf-error.log >> check-setup-310.log
    echo '======== End of Jupyter PDF error ========' >> check-setup-310.log
fi
if [ -s jupyter-webpdf-error.log ]; then
    echo '' >> check-setup-310.log
    echo '======== You had the following errors during Jupyter WebPDF generation ========' >> check-setup-310.log
    cat jupyter-webpdf-error.log >> check-setup-310.log
    echo '======== End of Jupyter WebPDF error ========' >> check-setup-310.log
fi
if [ -s jupyter-html-error.log ]; then
    echo '' >> check-setup-310.log
    echo 'You had the following errors during Jupyter HTML generation:' >> check-setup-310.log
    cat jupyter-html-error.log >> check-setup-310.log
    echo '======== End of Jupyter HTML error ========' >> check-setup-310.log
fi
# -f makes sure `rm` succeeds even when the file does not exists
