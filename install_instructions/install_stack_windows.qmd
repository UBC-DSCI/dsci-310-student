...
> **Note:** Some Windows 11 users might have Windows Terminal installed by default, 
> and so it might appear that nothing is installed when they follow these instructions. 
> To confirm, hit `Windows Key` and then type `terminal` and confirm the terminal is there. 
...
To avoid having to open the separate Miniforge Prompt every time we want to use Python, we can make it available from the (Git Bash) terminal, which is what we will be using most of the time. To set this up, open the "Miniforge Prompt" again and type:
...
> ```
>
> Delete the line that reads the following (it should be line 13, but could be elsewhere):
>
> ```
> PS1="$PS1"'\n'       # new line
> ```
...
```R
"C:/Users/florencia/R/win-library/4.5" "C:/Program Files/R/R-4.5.2/library"
```
...
Go to <http://cran.r-project.org/bin/windows/Rtools/> and download the latest version (e.g., rtools45-6691-6492.exe).
...
and it might already contain quite a few shortcuts.
For some, the right side might be completely blank. 
We're going to add two more shortcuts,
by pasting a text snippet.

If the right side is completely blank, paste the following code:
```json
{"shortcuts":[
]}
```

Whether or not there are already settings, or you pasted the code above, now go ahead and create a new line just after the line that says `"shortcuts": [`
...
> **Note:** Pay attention that due to the Windows settings suggested in this installation you *might* have to write  `quarto.cmd` instead of `quarto` to run Quarto commands. Try using `quarto` first, and if doesn't work, then use `quarto.cmd`. Read more [here](https://community.rstudio.com/t/bash-quarto-command-not-found/144187/3).
